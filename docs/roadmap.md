...
- **Priority:** P2
- **Demo-path:** no
- **ADR:** ADR-0128 (trade-ticket object model, record flow, ninth Area);
  ADR-0129 (provider channel — only Stage A in scope, Stage C behind the
  legal-counsel gate); ADR-0130 (cash exemption from the non-negativity
  guard)
...
**Progress.** Scope was fixed by ADR-0128/0129 (accepted 2026-08-27).
Landed: schema and substrate `b034` (S1, 2026-08-31); the emission engine
— order kinds, reported kinds incl. U-NEW creation-at-booking, reversal
with five typed causes (S2, 2026-09-01); the ninth Area shell (S3,
2026-09-01); the S4a order composer with ticket preview and the
sanctioned `create_cash_position` seam (2026-09-04); the S4b
new-instrument wizard over the gesture endpoints (2026-09-04); the S4c
reported-flow surfaces — secondary sale, primary commitment, secondary
purchase (2026-09-05→06); the S5 blotter and history bodies with a single
resume GET, cancel- and reversal-with-reason, and the live negative-cash
area indicator (2026-09-07); the S6 pre-trade impact panel on the blotter
row, over the ADR-0104 overlay pipeline via the `services/planning_desk/`
seam (2026-09-07); and the S7 ADR-0129 Stage-A contract (2026-09-07):
versioned envelope/fill schemas, signed directory format v1 (Ed25519),
fail-closed publishing-key placeholder, pre-fill seam, contract tests —
no service, no flag (D-flag). **All seven strands landed; Alembic head is
still `b034` (S1 was the track's only migration).** The track closing
report (fast-follow register F-1…F-8, Stage B as a separate project,
...
**Progress (2026-09-11).** **SB-3a shipped** — `services/provider_channel/ring.py`
selects the publishing key by the document's `publishing_key_id` from the
shipped ring, refuses unknown ids before any signature check, and reports
successor notices (B-D-14 addendum; `docs/reports/PB-1b-report.md`).
**SB-4 shipped** — `provider_channel.enabled` declared in the scoped-settings
taxonomy with its Admin card; ADR-0131 (annex to ADR-0112 §3); nothing reads
the switch yet (`docs/reports/PB-1d-report.md`). **MB-1 decided** — the
suggestion list's place in the Transactions UI is B-D-25; the
`asset_classes` matching rule is B-D-26. Next: SB-5 (`pynacl`, sealed box),
then publication with the release, SB-3b (fetch/cache client, verified
against the deployed directory), SB-6 (filter, export, timer), B-1
milestone review.

**Progress (2026-09-16).** **Release `2026.09.0`** cut 2026-09-13 without
the publication; first publication of `directory_version 1` moves to
`2026.09.1` (B-D-24 addendum). **SB-5 shipped** (`395b157`) —
`services/provider_channel/sealed_box.py` (libsodium-compatible
`x25519-sealed-box` over `pynacl>=1.6.2`, new runtime dependency per
B-D-12), the versioned export contract `ExportPayload` / `ExportEnvelope`
(`EXPORT_SCHEMA_VERSION = 1`) in `schemas.py`, and `export.py` with
`seal_export` against a verified `ProviderEntry`; provider-channel suite
101 → 155; known-answer fixture with a throwaway key pair (B-D-12
addendum; `docs/reports/PB-1e-report.md`). Housekeeping landed 2026-09-11
and -13: PB-H1 (dormant-pill note), PB-H3 (rerun-once exception retired
in `full-suite.yml`, redundant asyncio marker dropped), full-suite
baseline 4,927 passed at `2a2be75`
(`docs/reports/full-suite-2026-09-11-report.md`). Next: full-suite home
run, SB-3b (fetch/cache client, verified against the deployed directory
once `2026.09.1` publishes), SB-6 (filter, export gesture, timer, panel),
B-1 milestone review; libsodium.js parity check parked for B-3.

**Progress (2026-09-17).** **SB-3b shipped** (`d0393bb` library, `de1b0cb`
CLI) — new package `services/provider_directory/` (fetch with conditional
GET, monotonic acceptance with eight outcomes, file cache under `DATA_DIR`
with atomic write and re-verify-on-read, `Provenance` for the panel; 89
DB-free tests) and the operator instrument `portfoliflow directory-refresh`
/ `directory-status` (24 tests). The fetch client lives outside
`services/provider_channel/` because the C-2 contract forbids `httpx` and
`core.*` there (B-D-13/15/17 addenda; `docs/reports/PB-1f-report.md`,
`PB-1g-report.md`). Deployed-directory verification deferred to the
publication-day walk with `2026.09.1`. Next: pyright island housekeeping
done, home run green (5,109 at `3a34537`); SB-6 (`enabled` consumer,
24-hour timer, refresh gesture, panel, suggestion filter, export
gesture), B-1 milestone review.
...
| 2026-09-10 | **Provider channel Stage B — decision record extended by PB-D2; #067 progress noted.** `docs/concepts/provider-channel-stage-b-decisions.md`: **B-D-23** (`.sig` file encoding: 128 lowercase hex + LF, `text/plain`, strict decode) and **B-D-24** (first publication of `directory_version 1` ships with the next PortfoliFLOW release; gates SB-3b only) added; addenda to **B-D-7** (`pinkernelle-infrastructure` local-only, `git bundle` backup, `PORTFOLIFLOW_SRC` convention), **B-D-10** (daughter chats per strand from B-2 on) and **B-D-14** (key ring delivered by SB-1); infra tool-level decisions **D-SB2-1…16** listed by reference; §3 ceremony status, §6 dated-tests lesson, §7 coordinates re-verified post-SB-1, §8 items 2/4 struck and item 5 (publication) added. `docs/concepts/provider-directory-format.md`: §4 signature-file-encoding bullet, §11 pointer to the record. Report `docs/reports/PB-D2-report.md`. Documentation-only. *(Row added by PB-D3; the PB-D2 addendum that should have added it was not applied.)* |
| 2026-09-11 | **Provider channel Stage B — SB-3a and SB-4 shipped; MB-1 decided; record extended by PB-D3.** Code: `services/provider_channel/ring.py` (`verify_directory_with_ring`, `RingVerification`, `UnknownPublishingKeyId`; provider-channel suite 85 → 101, `test_contract.py` 22 unchanged) and the `provider_channel` declaration in `services/credential_vault/taxonomy.py` with its Admin-card copy in `web/routes/provider_credentials.py` (`env_fallback=False`, `optional=True`, no env link; card pill "dormant"). ADR: **0131** *Provider Channel Opt-In Switch in the Scoped-Settings Taxonomy* (annex to ADR-0112 §3; next free 0132). Record: **B-D-25** (suggestion list in the Transactions UI — `Providers…` row gesture, panel in the D-6b cell, `proposed`/`approved` only, provenance + Refresh in the panel foot, `[TEST]` chip and note, absent when disabled, matches first) and **B-D-26** (`asset_classes` hints match tenant `AssetClass.code` literally); addenda to B-D-14 (SB-3a) and B-D-21 (SB-4); D-SB2-16 corrected (window opens on day 61); §6 verify-first lesson; §7 coordinates re-verified post-SB-4. Reports `docs/reports/PB-1b-report.md`, `PB-1d-report.md`, `PB-D3-report.md`. |
| 2026-09-16 | **Provider channel Stage B — SB-5 shipped; release `2026.09.0` cut without the publication; record extended by PB-D4.** Code (`395b157`): `services/provider_channel/sealed_box.py` (X25519 + XSalsa20-Poly1305 sealed box over `pynacl>=1.6.2`, new runtime dependency per B-D-12), `export.py` (`seal_export`, `export_plaintext_bytes` reusing `directory.canonical_bytes`), `schemas.py` (`ExportPayload`, `ExportEnvelope`, `EXPORT_SCHEMA_VERSION = 1`, `EXPORT_DIRECTIONS`), a positive import allow-list test, a throwaway known-answer fixture; provider-channel suite 101 → 155, `__all__` 55 → 76. Housekeeping PB-H1/PB-H3 and the full-suite baseline report (4,927 at `2a2be75`) landed 2026-09-11/13. Record: addenda to **B-D-10** (every remaining strand as a daughter chat with a ratified kickoff), **B-D-12** (SB-5 delivery; what is left to B-2) and **B-D-24** (publication moves to `2026.09.1`); §6 process additions (working ids never in repository documents, release process, closing-report headings, Starlette 422 replacement rule, housekeeping and sealed-box lessons); §7 coordinates re-verified post-SB-5. `docs/concepts/provider-directory-format.md`: the `encryption_public_key` row and the §11 sealed-box bullet no longer say that nothing encrypts. Report `docs/reports/PB-D4-report.md`. Documentation-only. |
| 2026-09-17 | **Provider channel Stage B — SB-3b shipped; record extended by PB-D5.** Code (`d0393bb`, `de1b0cb`): `services/provider_directory/` (`fetch`, `cache`, `provenance`, `refresh`; `refresh_directory` with the eight-status vocabulary incl. `refused_unknown_key`, ETag conditional GET, 10 s timeout, degradation to cache; atomic three-file cache under `<data_dir>/provider_directory/`, re-verified on every read; 89 DB-free tests), `cli/directory.py` (`directory-refresh`, `directory-status`, `--json`, exit codes 0/2/3/4/5; 24 tests), one docstring in `services/provider_channel/directory.py`; no migration, no ADR, no dependency change. Record: addenda to **B-D-13** (client package outside the pure package; tampered cache = no cache; expired-but-signed cache stays the downgrade baseline), **B-D-15** (eight outcomes, `refused_unknown_key`, never-raises contract) and **B-D-17** (the *how* delivered, the *when* is SB-6's; URL as code constant; operator instrument; deployed verification deferred to `2026.09.1`); §6 lessons (7)–(12); §7 coordinates re-verified post-SB-3b. Report `docs/reports/PB-D5-report.md`. Documentation-only. |
| 2026-09-17 | **Provider channel Stage B — pyright island set and full-suite baseline (PB-H5, PB-D6).** `[tool.pyright] include` gains `services/provider_channel` and `services/provider_directory` (ADR-0109 §3 island-by-island; bare `pyright` 0 errors over four islands); two pre-existing errors in `tests/services/provider_channel/test_directory.py` cleared (`dict[str, Any]` splat annotation; code-scoped `pyright: ignore` on the deliberate ring-assignment refusal). Full-suite baseline 2026-09-17: 5,118 selected, **5,109 passed**, 0 failed, 8 skipped, 1 xfailed at `3a34537` (`docs/reports/full-suite-2026-09-17-report.md`); +188 over 2026-09-11; the 13 pytest asyncio-marker warnings are 0 (PB-H3 confirmed). Finding: six sample-workbook tests (v21/v24 limits/AUM roundtrip, Phase-7 import wiring, benchmark import route) skip on every machine and in CI because `data/sample/` is gitignored — open, outside Stage B. Record §7 updated. Report `docs/reports/PB-D6-report.md`. Documentation-only. |
